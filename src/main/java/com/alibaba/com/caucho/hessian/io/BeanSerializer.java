...
extends AbstractSerializer {
    ...
    private Method findSetter(Method[] methods, String getterName, Class arg) {
        ...
        for (int i = 0; i < methods.length; ++i) {
            Class<?>[] params;
            Method method = methods[i];
            if (!method.getName().equals(setterName) || !method.getReturnType().equals(Void.TYPE) || (params = method.getParameterTypes()).length != 1 || !params[0].equals((Object)arg)) continue;
            return method;
        }
        return null;
    }
