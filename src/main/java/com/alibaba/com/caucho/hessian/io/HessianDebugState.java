...
implements Hessian2Constants {
    ...
    extends State {
        ...
        State next(int ch) {
            if (this._lengthIndex < 2) {
                this._length = 256 * this._length + (ch & 255);
        ...
        State next(int ch) {
            ...
            switch (this._utfState) {
                case 0: {
                    ...
                    if (ch < 224) {
                        this._ch = (char)((ch & 31) << 6);
                        this._utfState = 1;
                        break;
                    }
                    this._ch = (char)((ch & 15) << 12);
                    this._utfState = 2;
                    break;
                }
                case 1: 
                case 3: {
                    this._ch = (char)(this._ch + (ch & 63));
                ...
                case 2: {
                    this._ch = (char)(this._ch + (char)((ch & 63) << 6));
                    this._utfState = 3;
                }
            }
            if (this._length == 0 && this._isLastChunk) {
    ...
    extends State {
        ...
        @Override
        State next(int ch) {
            this._value = 256 * this._value + (ch & 255);
            if (++this._length == 4) {
    ...
    extends State {
        ...
        @Override
        State next(int ch) {
            this._value = 256L * this._value + (long)(ch & 255);
            if (++this._length == 8) {
    ...
    extends State {
        ...
        @Override
        State next(int ch) {
            this._value = this._isFirst ? (int)((byte)ch) : 256 * this._value + (ch & 255);
            this._isFirst = false;
            if (++this._length == 4) {
    ...
    abstract class State {
        ...
        void printIndent(int depth) {
            if (HessianDebugState.this._isNewline) {
                for (int i = HessianDebugState.access$100((HessianDebugState)HessianDebugState.this); i < this.depth() + depth; ++i) {
        ...
        void print(int depth, String string) {
            ...
            if (p > 0) {
                ...
                HessianDebugState.this._column = HessianDebugState.this._column + string.length();
            }
        }
